...
export function buildMediaRunRequestFromBrief(brief = {}, { profiles, formats, runId } = {}) {
  ...
  // An explicit counter-case attached to the brief (receipt-backed when it cites a source).
  if (!counterFound && brief.counterCase) {
    const cc = brief.counterCase;
    const ccCite = (Array.isArray(cc.citationIds) ? cc.citationIds : []).map((id) => citeById.get(id)).find(Boolean) || null;
    evidence_records.push({
      evidence_record_id: cc.id || "ev_counter_case",
      claim: clean(typeof cc === "string" ? cc : (cc.body || cc.text || "")),
      support_level: "partially_supported",
      role: "counter_case",
      citation_id: clean(ccCite?.citationId || ""),
      receipt: ccCite ? { publisher: ccCite.publisher, title: ccCite.title, date: ccCite.date || "", url: ccCite.url } : ((cc && cc.receipt) || {}),
      locator: ccCite ? { captured_at: ccCite.date || "", section: ccCite.location } : {}
    });
    counterFound = true;
  }

  const read = {
    // source_run_id must reference an actual runs(id); never substitute the case id,
    // which would violate the read_snapshots / media_runs foreign keys on persistence.
    // The active run's id is threaded in explicitly since the answer brief does not carry it.
    source_run_id: clean(runId || brief.runId || ""),
  ...
  // Mirror the server gate (startMediaRun) so the workspace never enables a run the server
  // will refuse: a media run must trace to a real investigation run.
  if (!read.source_run_id) return { ok: false, reason: "source_run_required", read };
...
export function describeMediaRunBlocker(reason) {
  ...
  if (reason === "source_run_required") return "Complete an investigation run first — media must trace to a saved run.";
