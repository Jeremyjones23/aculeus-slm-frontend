...
export async function runSubstanceLock({ render = {}, snapshot = {}, atoms = [], authToken = "", verifyModelId, renderModelId } = {}) {
  ...
  if (providerConfigured(vModel)) {
    try {
      const text = await callProvider({ model: vModel, messages: buildSubstanceLockMessages(render, snapshot) });
  ...
  // The verifier must run on a DIFFERENT model than the renderer; if the model ids resolve
  // to the same model (e.g. all roles falling back to ACULEUS_MODEL_ID), the gestalt check
  // shares the writer's blind spots and cannot certify the render for publication.
  const cross_model = vModel !== rModel;
  return {
    role: "substance_lock",
    model_id: vModel,
    ...
    tier2,
    blocked,
    // Passing requires no block issues, that the gestalt pass actually ran, AND that the
    // verifier ran on a different model than the renderer.
    substance_lock_passed: !blocked && gestalt.gestalt_checked && cross_model,
    publish_ready: !blocked && gestalt.gestalt_checked && cross_model,
    bindings,
...
export async function verifyAndRepair({
  ...
  const lock = async (r) => {
    if (providerConfigured(verifyModel)) spent += callCostCents;
  ...
  if (verdict.blocked) {
    ...
    // Not blocked, but cannot publish without a human. Report the most operative reason:
    // the gestalt pass never ran, or (if it did) the verifier shared the renderer's model.
    status = "needs_human";
    escalated_to = !verdict.gestalt_checked
      ? "human_review_gestalt_not_run"
      : (verdict.cross_model === false ? "human_review_same_model_verifier" : "human_review");
  } else {
    status = "verified";
  }
...
// The hard gate fails CLOSED: any issue the gestalt verifier returns blocks unless the
// model explicitly downgrades it to warn/info, and the enumerated severe types can never
// be downgraded. A missing/unknown severity on a flagged issue must never pass as a warning.
export function normIssues(list) {
  return (Array.isArray(list) ? list : [])
    .map((i) => {
      const issue_type = clean(i.issue_type);
      const sev = clean(i.severity);
      const downgradable = (sev === "warn" || sev === "info") && !BLOCK_ISSUE_TYPES.has(issue_type);
      return { issue_type, severity: downgradable ? sev : "block", message: clean(i.message) };
    })
