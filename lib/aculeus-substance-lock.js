...
export async function runSubstanceLock({ render = {}, snapshot = {}, atoms = [], authToken = "", verifyModelId, renderModelId } = {}) {
  ...
  if (gatewayConfigured(authToken)) {
    try {
      const text = await callGateway({ model: vModel, messages: buildSubstanceLockMessages(render, snapshot), authToken });
...
export async function verifyAndRepair({
  ...
  const lock = async (r) => {
    if (gatewayConfigured(authToken)) spent += callCostCents;
