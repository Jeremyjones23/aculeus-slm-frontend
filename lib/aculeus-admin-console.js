export function buildAdminRunSummary(store = {}) {
  ...
  const providerBudgetTotals = buildProviderBudgetTotals(runs);
  return {
    ...
    provider_budget_totals: providerBudgetTotals,
    rows,
    public_safe: true
  };
}
...
export function validateAdminRunSummary(summary = {}) {
  ...
  if (!summary.provider_budget_totals || !Array.isArray(summary.provider_budget_totals.by_provider)) issues.push("admin summary missing provider budget totals");
...
function providerCostForRun(run = {}) {
  ...
  const providerEntries = ledger.filter(isProviderEntry);
...
function buildProviderBudgetTotals(runs = []) {
  const byDay = new Map();
  const byWorkspace = new Map();
  const byProvider = new Map();
  for (const run of runs) {
    const workspaceId = run.workspace_id || run.workspaceId || "workspace_default";
    for (const entry of Array.isArray(run.ledger) ? run.ledger : []) {
      if (!isProviderEntry(entry)) continue;
      const spend = extractSpend(entry);
      const cap = extractCap(entry);
      const calls = entry.result_summary?.provider_calls_executed || entry.result_summary?.paid_spend_incurred ? 1 : 0;
      const day = String(entry.created_at || run.updatedAt || run.createdAt || new Date().toISOString()).slice(0, 10);
      addBudgetRow(byDay, day, { spend, cap, calls });
      addBudgetRow(byWorkspace, workspaceId, { spend, cap, calls });
      addBudgetRow(byProvider, providerName(entry), { spend, cap, calls });
    }
  }
  return {
    by_day: mapBudgetRows(byDay, "day"),
    by_workspace: mapBudgetRows(byWorkspace, "workspace_id"),
    by_provider: mapBudgetRows(byProvider, "provider")
  };
}
...
function providerName(entry = {}) {
  ...
  return String(
    entry.provider ||
    entry.result_summary?.provider ||
    entry.result_summary?.provider_name ||
    candidates.find((candidate) => candidate.provider)?.provider ||
    "provider"
  ).toLowerCase();
}

function addBudgetRow(map, key, { spend, cap, calls }) {
  const id = String(key || "unknown");
  const current = map.get(id) || { estimated_spend_usd: 0, provider_cap_usd: 0, provider_call_count: 0 };
  current.estimated_spend_usd += Number(spend || 0);
  current.provider_cap_usd += Number(cap || 0);
  current.provider_call_count += Number(calls || 0);
  map.set(id, current);
}

function mapBudgetRows(map, keyName) {
  return [...map.entries()]
    .map(([key, value]) => ({
      [keyName]: key,
      estimated_spend_usd: roundMoney(value.estimated_spend_usd),
      provider_cap_usd: roundMoney(value.provider_cap_usd),
