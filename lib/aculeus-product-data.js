...
export function buildAnswerBriefFromCase(casePacket) {
  ...
  return {
    ...
    counterCase: buildCounterCase(casePacket, citations),
    citations
  };
}
...
function buildCounterCase(casePacket, citations) {
  // The Read's "strongest case against it": prefer an explicit counter-case, then a
  // finding's what-it-does-not-prove, then a claim-ledger limitation. Receipt-backed when
  // the limiting source is among the cited sources. Returns null when the record supplies none.
  const dossier = casePacket?.dossier || {};
  const explicit = casePacket?.counterCase || dossier.counterCase || dossier.strongestCaseAgainst;
  const findings = Array.isArray(dossier.findings) ? dossier.findings : [];
  const ledger = Array.isArray(dossier.claimLedger) ? dossier.claimLedger : [];
  const candidate = (explicit && { text: typeof explicit === "string" ? explicit : explicit.body || explicit.text, sourceIds: explicit.sourceIds })
    || findings.map((f) => ({ text: f.whatItDoesNotProve, sourceIds: f.sourceIds })).find((x) => x.text && String(x.text).trim())
    || ledger.map((l) => ({ text: l.limitation, sourceIds: l.sourceIds })).find((x) => x.text && String(x.text).trim())
    || null;
  if (!candidate || !String(candidate.text).trim()) return null;

  const sourceIds = Array.isArray(candidate.sourceIds) ? candidate.sourceIds : [];
  const citationIds = citations.filter((c) => sourceIds.includes(c.sourceId)).map((c) => c.citationId).slice(0, 2);
  return {
    id: "counter_case",
    title: "Strongest case against",
    body: String(candidate.text).replace(/\s+/g, " ").trim(),
    citationIds
  };
}
