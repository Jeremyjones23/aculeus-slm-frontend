...
export function buildReadSnapshot(read = {}, version = 1) {
  ...
  const snapshot = {
    source_run_id: clean(read.source_run_id || read.sourceRunId || read.runId || ""),
    case_id: clean(read.case_id || read.caseId || ""),
    snapshot_version: Number(version) || 1,
...
function hasCounterCase(read) {
  if (read.counter_case || read.counterCase) return true;
  return promotedEvidence(read).some(isQualifier);
}
