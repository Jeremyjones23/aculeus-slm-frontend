...
export function createLocalProductRepository({ storeDir = resolveStoreDir() } = {}) {
  ...
  function reviewTrainingTrace(traceId, review = {}) {
    const store = readStore();
    for (const run of store.runs) {
      const index = (run.trainingTraces || []).findIndex((trace) => trace.trace_id === traceId);
      if (index < 0) continue;
      const updated = normalizeReviewedTrainingTrace(run.trainingTraces[index], review);
      run.trainingTraces[index] = updated;
      run.updatedAt = updated.reviewed_at;
      writeStore(store);
      return updated;
    }
    throw new Error(`Training trace not found: ${traceId}`);
  }
...
export function createDatabaseProductRepository() {
  ...
  async function reviewTrainingTrace(traceId, review = {}) {
    const rows = await query("select id, run_id, payload from training_traces where id = $1 limit 1", [traceId]);
    if (!rows[0]) throw new Error(`Training trace not found: ${traceId}`);
    const updated = normalizeReviewedTrainingTrace(rows[0].payload, review);
    await query(
      `update training_traces
       set payload = $2::jsonb,
           ...
           public_safe = $3
       where id = $1`,
      [traceId, JSON.stringify(updated), updated.public_safe !== false]
    );
    if (rows[0].run_id) {
      const run = await getRun(rows[0].run_id);
      if (run) {
        run.trainingTraces = (run.trainingTraces || []).map((trace) => trace.trace_id === traceId ? updated : trace);
        run.updatedAt = updated.reviewed_at;
        await saveRun(run);
      }
    }
...
export function reviewTrainingTrace(traceId, review) {
  return getProductRepository().reviewTrainingTrace(traceId, review);
}
...
function normalizeReviewedTrainingTrace(trace, review = {}) {
  const now = new Date().toISOString();
  const decision = normalizeTraceReviewDecision(review.decision || review.review_decision);
  const approved = decision === "approve_for_training";
  return sanitizePublicObject({
    ...trace,
    reviewed: true,
    reviewed_at: now,
    reviewed_by: review.reviewed_by || review.reviewer_id || "aculeus_reviewer",
    review_decision: decision,
    review_note: String(review.note || review.review_note || "").slice(0, 600),
    review_status: approved ? "approved_for_sft_preference_pool" : decision === "reject_for_training" ? "rejected_for_training" : "needs_trace_review_changes",
    training_conversion_allowed: approved,
    sft_preference_pool_eligible: approved,
...
function normalizeTraceReviewDecision(decision) {
  const value = String(decision || "").toLowerCase();
  if (["approve", "approved", "approve_for_training", "approved_for_training"].includes(value)) return "approve_for_training";
  if (["reject", "rejected", "reject_for_training", "rejected_for_training"].includes(value)) return "reject_for_training";
