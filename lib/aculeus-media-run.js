...
export async function startMediaRun({
  ...
  // A media run must trace to a real investigation run (read_snapshots / media_runs FK to
  // runs(id)). Refuse before spending any model calls if there is no source run.
  if (!snapshot.source_run_id) return { ok: false, reason: "source_run_required" };
  ...
  const media_run = {
    media_run_id: `media_${Date.now()}_${randomUUID().slice(0, 8)}`, // unique even for same-ms creates
    source_run_id: snapshot.source_run_id,
    read_snapshot_id: snapshot.snapshot_id, // run-scoped durable snapshot row id
    read_snapshot_content_hash: snapshot.content_hash, // substance fingerprint the render pins
