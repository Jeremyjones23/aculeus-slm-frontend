...
function modelConfigured(authToken = "") {
  return Boolean(gatewayToken(authToken));
}

function gatewayToken(authToken = "") {
  return authToken
...
async function generateModelAnswer(question, casePacket, gatewayAuthToken = "") {
  const modelId = configuredModelId();
  const response = await fetch("https://ai-gateway.vercel.sh/v1/chat/completions", {
    method: "POST",
    headers: {
      "authorization": `Bearer ${gatewayToken(gatewayAuthToken)}`,
...
async function generateInvestigationReview(casePacket, gatewayAuthToken = "") {
  const modelId = configuredModelId();
  const response = await fetch("https://ai-gateway.vercel.sh/v1/chat/completions", {
    method: "POST",
    headers: {
